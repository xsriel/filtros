package com.example.filtros.Utils
...
object BitmapUtils {
    ...
    private fun storeThumbnail(contentResolver: ContentResolver, source: Bitmap?, id:Long, width: Float, height: Float, microKind: Int) : Bitmap? {
        val matrix = Matrix()
        val scaleX: Float = width/source!!.width
        val scaleY: Float = height/source!!.height

        matrix.setScale(scaleX, scaleY)

        val thumb: Bitmap = Bitmap.createBitmap(source, 0, 0, source.width, source.height, matrix, true)
